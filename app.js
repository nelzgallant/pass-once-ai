...
function show(id) {
  ["auth", "setup", "dash", "quiz", "tutorView"].forEach(x => $(x).classList.add("hidden"));
...
function normalizeLevel(level) {
  return String(level || "JSS 1").replace(/\s+/g, "").toUpperCase();
}
...
$("authBtn").onclick = async () => {
  ...
  try {
    ...
    const result = signup
      ? await sb.auth.signUp({ email, password, options: { data: { full_name: $("name").value.trim() } } })
      : await sb.auth.signInWithPassword({ email, password });
    if (result.error) throw result.error;
    if (signup && !result.data.session) {
      message("Account created. Check your email if email confirmation is enabled, then log in.");
      return;
    }
    await load();
  } catch (e) {
...
$("saveProfile").onclick = async () => {
  try {
    ...
    const p = {
      id: user.id,
      full_name: $("name").value.trim() || user.user_metadata?.full_name || user.email.split("@")[0],
...
async function findClass() {
  ...
  if (error) throw error;
  return (data || []).find(c => normalizeLevel(c.name) === wanted) || null;
}
...
async function renderDash() {
  ...
  const { data: attempts, error: attemptsError } = await sb
    .from("quiz_attempts").select("score,total").eq("user_id", user.id);
  ...
  if (!classRow) {
    $("subjects").innerHTML = `<div class="card">We have not loaded this class into the database yet. Choose JSS1 for the current practice demo.</div>`;
    return;
  }
  ...
  // If curriculum_subjects has not yet been mapped for this class, use the subjects table.
  if (!unique.length) {
    const fallback = await sb.from("subjects").select("id,name").order("id");
    if (fallback.error) throw fallback.error;
    unique.push(...(fallback.data || []));
  }

  $("subjects").innerHTML = unique.map(s =>
    `<button class="card subject" data-id="${s.id}" data-name="${escapeHtml(s.name)}">📚 ${escapeHtml(s.name)}</button>`
...
async function start(subjectId, subjectName) {
  ...
  const { data, error } = await sb
    ...
    .eq("language_code", "en")
    .in("topic_id", await topicIds(subjectId, classRow.id));

  if (error) {
    $("question").textContent = "Could not load questions.";
    $("explain").textContent = error.message;
  ...
  if (!currentQuestions.length) {
    $("question").textContent = `No practice questions are loaded for ${subjectName} yet.`;
    $("options").innerHTML = `<p>Add questions for this subject in Supabase and they will appear here automatically.</p>`;
    return;
  }
  renderQ();
}
...
async function topicIds(subjectId, classId) {
  const { data, error } = await sb
    .from("topics").select("id").eq("subject_id", subjectId).eq("class_id", classId).order("id");
  if (error) throw error;
...
function renderQ() {
  ...
  $("count").textContent = `${currentIndex + 1}/${currentQuestions.length}`;
  $("bar").style.width = ((currentIndex + 1) / currentQuestions.length * 100) + "%";
  $("question").textContent = q.question;
  $("options").innerHTML = opts.map((x, n) =>
    `<button class="option" data-i="${n}">${String.fromCharCode(65+n)}. ${escapeHtml(x)}</button>`
  ).join("");
  $("explain").classList.add("hidden");
  $("next").classList.add("hidden");
  answered = false;
...
function correctIndex(q) {
  const value = String(q.correct_answer || "").trim().toUpperCase();
  if (["A","B","C","D"].includes(value)) return value.charCodeAt(0) - 65;
  if (["1","2","3","4"].includes(value)) return Number(value) - 1;
  return -1;
}
...
function answer(n) {
  ...
  if (right >= 0) buttons[right].classList.add("correct");
  if (n === right) currentScore++;
  else if (n >= 0) buttons[n].classList.add("wrong");
  $("explain").textContent = (n === right ? "✅ Correct! " : "❌ Not quite. ") + (q.explanation || "Review the lesson and try again.");
  $("explain").classList.remove("hidden");
  $("next").textContent = currentIndex < currentQuestions.length - 1 ? "Next Question" : "Finish Practice";
...
async function nextQuestion() {
  ...
  try {
    ...
    $("options").innerHTML = `<div class="result card"><h2>${currentScore}/${currentQuestions.length}</h2><p>Your result has been saved to your Pass Once AI progress.</p><button class="primary" id="resultDash">Back to Dashboard</button></div>`;
    $("explain").classList.add("hidden");
    $("next").classList.add("hidden");
    $("resultDash").onclick = renderDash;
  } catch (e) {
...
$("logout").onclick = async () => {
  ...
  user = null; profile = null; classRow = null;
  $("logout").classList.add("hidden");
  show("auth");
};
...
$("tutor").onclick = () => {
  show("tutorView");
  $("chat").innerHTML = '<div class="bubble">Hi! I am your Pass Once AI Tutor. Ask me a school question.</div>';
};
...
$("chatForm").onsubmit = e => {
  ...
  const reply = lower.includes("quadratic")
  ...
  $("chat").insertAdjacentHTML("beforeend", `<div class="bubble user">${safe}</div><div class="bubble">${escapeHtml(reply)}</div>`);
  $("ask").value = "";
};
